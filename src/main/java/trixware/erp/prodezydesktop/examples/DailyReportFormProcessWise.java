 ...
 * @author Rajesh
 */
public class DailyReportFormProcessWise extends javax.swing.JPanel {
...
//    HashMap<String, String> entity = null ;
    ...
    SingleProcessDE_FormProcessWise[] forms = null;
     ...
     */
    public DailyReportFormProcessWise () {
        initComponents ();
    ...
    public void loadComboBoxes () {
       ...
       if ( jComboBox1.getItemCount () == 0 ) {
           ...
           forms = new SingleProcessDE_FormProcessWise[ machines.size () ];
           loadDE_Label ();
           loadDE_Form ();
       }
                ...
                for ( int i = 0 ; i < records.length () ; i ++ ) {
                    ...
                    for ( int j = 0 ; j < records2.length () ; j ++ ) {
                        ...
                        if ( emp.get ( "REF_PROCESS_ID" ).toString ().equals ( emp2.get ( "PROCESS_ID" ).toString () ) ) {
                            jComboBox2.addItem( emp2.get ( "PROCESS_NAME" ).toString () );
...
//             BTC_NAME = selectedBtch.BTC_NAME ;
    ...
    public void loadProcessesForProduct_OLD(){
        ...
        String queryOne    = "SELECT PROCESS_ID, PROCESS_NAME FROM PROCESS_MASTER";
        ...
        String queryFour = "SELECT RR_ID, RR_DESC FROM rejection_reasons";
           ...
           // int selectedFG_ID =  products.get(jComboBox1.getSelectedIndex ()).FG_ID;
            result2 = DB_Operations.executeSingle ( "SELECT distinct FG_ID, PART_NAME FROM finished_goods a INNER JOIN FG_PROCESS_MACH_MAP b on a.FG_ID = b.REF_FG_ID AND REF_PROCESS_ID = "+ processes.get( jComboBox2.getSelectedIndex ()).PRC_ID +" order by FG_ID" );
           ...
           try {
            ...
            while ( result2.next () ) {
                ...
                prdr.FG_ID = Integer.parseInt ( result2.getString ( 1 ) );
                prdr.FG_PART_NO = result2.getString ( 2 );
                products.add ( prdr );
                jComboBox1.addItem ( result2.getString ( 2 ) );
            }
        } catch ( SQLException e ) {
            System.out.println ( " Error 1 " + e.getMessage () );
            StaticValues.writer.writeExcel ( DailyReportFormProcessWise.class.getSimpleName () , DailyReportFormProcessWise.class.getSimpleName () , e.getClass ().toString () , Thread.currentThread ().getStackTrace ()[ 1 ].getLineNumber () + "" , e.getMessage () , StaticValues.sdf2.format ( Calendar.getInstance ().getTime () ) );
        }catch ( Exception e ) {
            System.out.println ( " Error 1 " + e.getMessage () );
        }finally{
                 try {
    ...
    public void loadComboBoxes_OLD () {
     ...
     try {
            result2 = DB_Operations.executeSingle ( queryOne );
            processes = new ArrayList<ProcessDR> ();
            ...
            while ( result2.next () ) {
                ...
                processes.add ( prcdr );
                jComboBox2.addItem ( result2.getString ( 2 ) );
            }
    ...
    public void loadDE_Form(  ){
        ...
        String recordExists ;
        SingleProcessDE_FormProcessWise abc = null ;
        ...
        for(int i=0; i<machines.size (); i++){
                ...
                //    ResultSet yesRecordExist = DB_Operations.executeSingle ( recordExists );
                    ...
                    abc = new SingleProcessDE_FormProcessWise ();
                    abc.setName ( "formno"+i );
                    abc.setMachineID ( machines.get(i).MC_ID+"" );
                    abc.setMachinename ( machines.get(i).MC_NAME );
                    
                    abc.setEmployee ( employees );
                   // abc.setMachine ( machines );
                   // abc.setUnits(products);
                    abc.setRejectionReasons (rej_reasons );
                    
                    abc.setUnits(uni);
                    abc.setUnit(UOM);
                    if (verifyFirstOrOnlyProcess(products.get ( jComboBox1.getSelectedIndex () ).FG_ID ,  processes.get ( jComboBox2.getSelectedIndex () ).PRC_ID )) {
                    try {
...
//                jScrollPane1.setBounds( 50, (forms.length*60+175), 1250, 250 );
    ...
    public void loadDE_Label( ){
                ...
                SingleProcessDE_LabelsProcess abc = new SingleProcessDE_LabelsProcess ();
    ...
    private void initComponents() {
        ...
        add(jButton1);
        jButton1.setBounds(1160, 370, 70, 30);
        ...
        add(jButton2);
        jButton2.setBounds(1240, 370, 70, 30);
        ...
        add(jTextField1);
        jTextField1.setBounds(150, 20, 1, 30);
        ...
        add(jTextField2);
        jTextField2.setBounds(20, 20, 1, 30);

        jLabel2.setFont(new java.awt.Font("Leelawadee UI", 0, 12)); // NOI18N
        jLabel2.setText("Select Process");
        add(jLabel2);
        jLabel2.setBounds(320, 0, 90, 20);
        ...
        try {
            ...
            if (  ! result2.contains ( "\"101\":\"Report record\\/s not found.\"" ) &&  ! result2.contains ( "\"OPENING\":null,\"RECEIVED\":null,\"USED\":null,\"CLOSING\":null" ) ) {
                ...
                if ( verifyFirstOrOnlyProcess ( productId , processId ) ) {
                    ...
                    RMStockUpdatedOnfirstProcess = batch.insertIntoRMStock ( batches.get ( jComboBox3.getSelectedIndex () ).RMID , batches.get ( jComboBox3.getSelectedIndex () ).FGQty , quot , batches.get ( jComboBox3.getSelectedIndex () ).BTC_ID,Double.parseDouble(dataList.get ( 0 ).getQIn ()),Integer.parseInt(dataList.get (0).getQIn ()) , date );
    ...
    ArrayList<DailyDataEntryModel2> dataList;
    ...
    private void jButton1MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jButton1MouseClicked
        ...
        SingleProcessDE_FormProcessWise abc1 = null;
        String abc = "" ;
        dataList = new ArrayList<DailyDataEntryModel2>();
        ...
        for( int i=0; i<forms.length; i++ ){
          ...
          //  try{
                    if( checkIfEmpty ( abc1.getQIn () ,  abc1.fromHr, abc1.fromMin, abc1.toHr, abc1.toMin  ) ){
                       ...
                       // data.setMachine ( abc1.getMachine () );
                        data.setMachineID (abc1.getMachineID ());
                        data.setMachinename (abc1.getMachinename ());
         ...
         for(int i=0; i<dataList.size(); i++){
            ...
            try{
//                System.out.println("UOMIN "+i +" : "+dataList.get(i).getUomIn());
//                System.out.println("UOMOut "+i +" : "+dataList.get(i).getUomOut());
//                System.out.println("UOMOutRej "+i +" : "+dataList.get(i).getUomOutRej());
                ...
                try{
                    ...
                    if( d1.getTime() < d2.getTime() ){
                        ...
                        addToStores ( products.get ( jComboBox1.getSelectedIndex () ).FG_ID ,  processes.get ( jComboBox2.getSelectedIndex () ).PRC_ID  , Integer.parseInt ( dataList.get ( i ).getQout () ) , Integer.parseInt ( dataList.get ( i ).getRejected () )  , dataList.get ( i ).getPFromDate ()+" 00:00:00"  );
        ...
        // save the required details from datalist arraylist into op_wip2 for every process
            ...
            for(int i=0; i<dataList.size(); i++){
             ResultSet yesRecordExist = null;
                ...
                String recordExists ;
                boolean recordExist = false;
                recordExists = "select * from op_wip2 where  PRODUCT = "+products.get(jComboBox1.getSelectedIndex ()).FG_ID+" AND PROCESS_STAGE = "+processes.get(jComboBox2.getSelectedIndex ()).PRC_ID+" AND machine = "+dataList.get(i).getMachineID () + " AND empid="+employees.get(dataList.get(i).getEmployee ()).EMP_ID;
                ...
                if( yesRecordExist.isBeforeFirst ()){
                    ...
                    try {
                        ...
                        StaticValues.writer.writeExcel ( DailyReportFormProcessWise.class.getSimpleName () , DailyReportFormProcessWise.class.getSimpleName () , ex.getClass ().toString () , Thread.currentThread ().getStackTrace ()[ 1 ].getLineNumber () + "" , ex.getMessage () , StaticValues.sdf2.format ( Calendar.getInstance ().getTime () ) );
                    }
                }
                
                String getAccumulatedEntries = "select SUM(qtyin) as 'Qty In', SUM(qtyout) as 'Qty Out' from dailyreport where fgid="+products.get(jComboBox1.getSelectedIndex ()).FG_ID+" AND processid="+processes.get(jComboBox1.getSelectedIndex ()).PRC_ID+" AND machineid = "+dataList.get(i).getMachineID ()+" AND empid = "+employees.get(dataList.get(i).getEmployee ()).EMP_ID+" group by fgid" ; 
...
//        loadDE_Form (  );
    ...
    JOptionPane.showMessageDialog(null, "<html><size='06'>First add processName from Processes Master ");
}        // TODO add your handling code here:
...
//        
    ...
    // End of variables declaration//GEN-END:variables
     ...
     public boolean verifyLastProcess ( int productId , int processId ) {
        ...
        String addEmpAPICall = "processmachmaps?FG_ID=" + productId;
        String  result2 = WebAPITester.prepareWebCall ( addEmpAPICall , StaticValues.getHeader () , "" );
        HashMap<String , Object> map = new HashMap<String , Object> ();
        JSONObject jObject = new JSONObject ( result2 );
        Iterator<?> keys = jObject.keys ();
