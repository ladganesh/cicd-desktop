 ...
 * @author Rajesh
 */
public class OperatorEfficiencyReport extends javax.swing.JPanel {
...
//SimpleDateFormat sdf = null;
     ...
     */
    public OperatorEfficiencyReport () {
        initComponents ();
       ...
       // dateChooserCombo3.setText ( dateChooserCombo3.getText () );
       dateChooserCombo3.setText( "Apr 1, 2018 " ) ;
               dateChooserCombo3.setText( "Dec 31, 2018 " ) ;
...
//				dataSet, PlotOrientation.VERTICAL, true, true, false);
    ...
    public void loadEntriesAllSelected ( String reportType , int product , int process , int machine , int employee, int shift ) {
        ...
        try {
            ...
            if ( reportType.equalsIgnoreCase ( "Yearly" ) ) {
                ...
                //result = DB_Operations.executeSingle ("select (select PROCESS_NAME from PROCESS_MASTER where PROCESS_ID in (processid)) as 'Process' , (select PART_NAME from finished_goods where FG_ID in (fgid)) as 'Product',    showrdate  as 'Start Date',     SUM(qtyout) as 'Output Qty',     rdate,        SUM(qtyout/(stoptime-starttime)) as 'Speed', SUM(stoptime-starttime) as 'Hrs', SUM(qtyin-qtyout) as 'Balence' from dailyreport where rdate >= '"+"01-04-2018"+"' AND rtdate <= '"+"31-03-2018"+"' AND fgid="+product);
                result = DB_Operations.executeSingle ( "select     showrdate  as 'Start Date',     SUM(qtyout) as 'Output Qty'  from dailyreport where rdate >= '" + formatFromDate () + "' AND rdate <= '" + formatToDate () + "' AND fgid=" + product + " AND processid=" + process + " AND machineid=" + machine + " AND empid=" + employee+ " AND shift_id=" + shift );
                jTable1.setModel ( buildTableModel ( result , reportType , "xAxisLabel" ) );
                ...
                //result = DB_Operations.executeSingle ("select (select PROCESS_NAME from PROCESS_MASTER where PROCESS_ID in (processid)) as 'Process' , (select PART_NAME from finished_goods where FG_ID in (fgid)) as 'Product',  showrdate  as 'Start Date', SUM(qtyout) as 'Output Qty',    rdate,                  (qtyout/(stoptime-starttime)) as 'Speed', (stoptime-starttime) as 'Hrs', (qtyin-qtyout) as 'Balence' from dailyreport where fgid="+product+" group by rdate");
                result = DB_Operations.executeSingle ( "select   showrdate  as 'Start Date', SUM(qtyout) as 'Output Qty'  from dailyreport where  rdate >= '" + formatFromDate () + "' AND rdate <= '" + formatToDate () + "' AND fgid=" + product + " AND processid=" + process + " AND machineid=" + machine + " AND empid=" + employee + " AND shift_id=" + shift+ " group by rdate" );
                jTable1.setModel ( buildTableModel ( result , reportType , "xAxisLabel" ) );
                ...
                for ( int i = 0, j = i + 1 ; j < weeklyDates.size () ; i = i + 2 , j += 2 ) {
                    ...
                    //result = DB_Operations.executeSingle ("select (select PROCESS_NAME from PROCESS_MASTER where PROCESS_ID in (processid)) as 'Process' , (select PART_NAME from finished_goods where FG_ID in (fgid)) as 'Product',  showrdate  as 'Start Date', SUM(qtyout) as 'Output Qty',    rdate,                  (qtyout/(stoptime-starttime)) as 'Speed', (stoptime-starttime) as 'Hrs', (qtyin-qtyout) as 'Balence' from dailyreport where fgid="+product+" group by rdate");
                    result = DB_Operations.executeSingle ( "select   showrdate  as 'Start Date', SUM(qtyout) as 'Output Qty'  from dailyreport where  rdate >= '" + weeklyDates.get ( i ) + "' AND rdate <= '" + weeklyDates.get ( j ) + "' AND fgid=" + product + " AND processid=" + process + " AND machineid=" + machine + " AND empid=" + employee + " AND shift_id=" + shift);
                    fromWeekDate = weeklyDates.get ( i ).split ( " " )[ 0 ].substring ( 5 );
                    toWeekDate = weeklyDates.get ( j ).split ( " " )[ 0 ].substring ( 5 );
                    jTable1.setModel ( buildTableModel ( result , reportType , fromWeekDate + " to " + toWeekDate ) );
                }
                ...
                for ( int i = 0, j = i + 1, k = 0 ; j < months.size () && k < 12 ; i = i + 2 , j += 2 , k ++ ) {
                    ...
                    //result = DB_Operations.executeSingle ("select (select PROCESS_NAME from PROCESS_MASTER where PROCESS_ID in (processid)) as 'Process' , (select PART_NAME from finished_goods where FG_ID in (fgid)) as 'Product',  showrdate  as 'Start Date', SUM(qtyout) as 'Output Qty',    rdate,                  (qtyout/(stoptime-starttime)) as 'Speed', (stoptime-starttime) as 'Hrs', (qtyin-qtyout) as 'Balence' from dailyreport where fgid="+product+" group by rdate");
                    result = DB_Operations.executeSingle ( "select   showrdate  as 'Start Date', SUM(qtyout) as 'Output Qty'  from dailyreport where  rdate >= '" + months.get ( i ) + "' AND rdate <= '" + months.get ( j ) + "' AND fgid=" + product + " AND processid=" + process + " AND machineid=" + machine + " AND empid=" + employee + " AND shift_id=" + shift);
                    jTable1.setModel ( buildTableModel ( result , reportType , monthsTitles[ k ] ) );
                }
                ...
                for ( int i = 0, j = i + 1, k = 0 ; j < quarters.size () && k < 4 ; i = i + 2 , j += 2 , k ++ ) {
                    ...
                    //result = DB_Operations.executeSingle ("select (select PROCESS_NAME from PROCESS_MASTER where PROCESS_ID in (processid)) as 'Process' , (select PART_NAME from finished_goods where FG_ID in (fgid)) as 'Product',  showrdate  as 'Start Date', SUM(qtyout) as 'Output Qty',    rdate,                  (qtyout/(stoptime-starttime)) as 'Speed', (stoptime-starttime) as 'Hrs', (qtyin-qtyout) as 'Balence' from dailyreport where fgid="+product+" group by rdate");
                    result = DB_Operations.executeSingle ( "select   showrdate  as 'Start Date', SUM(qtyout) as 'Output Qty'  from dailyreport where  rdate >= '" + quarters.get ( i ) + "' AND rdate <= '" + quarters.get ( j ) + "' AND fgid=" + product + " AND processid=" + process + " AND machineid=" + machine + " AND empid=" + employee+ " AND shift_id=" + shift );
                    jTable1.setModel ( buildTableModel ( result , reportType , quarterTitles[ k ] ) );
                }
    ...
    public void loadEntriesAllMachine ( String reportType , int employee ) {
        ...
        try {
            ...
            if ( reportType.equalsIgnoreCase ( "Yearly" ) ) {
              ...
              // select (select PROCESS_NAME from PROCESS_MASTER where PROCESS_ID in (processid)) as 'Process', (select TG_OPT_HR from FG_PROCESS_MACH_MAP where REF_PROCESS_ID in (processid)) as 'Expected',  rdate, SUM( showToTime - showFromTime) as 'Work Hours', qtyout, ( qtyout / SUM(showToTime - showFromTime))*1.0 as 'Hourly Output' , (( qtyout / SUM(showToTime - showFromTime)) / (select TG_OPT_HR from FG_PROCESS_MACH_MAP where REF_PROCESS_ID in (processid)))*1.0 as 'Efficiency'  from dailyreport where empid = 1 group by rdate  
                result = DB_Operations.executeSingle ( "select (select PROCESS_NAME from PROCESS_MASTER where PROCESS_ID in (processid)) as 'Process', (select TG_OPT_HR from FG_PROCESS_MACH_MAP where REF_PROCESS_ID in (processid)) as 'Expected',  rdate, SUM( showToTime - showFromTime) as 'Work Hours', qtyout, ( qtyout / SUM(showToTime - showFromTime))*1.0 as 'Hourly Output' , (( qtyout / SUM(showToTime - showFromTime)) / (select TG_OPT_HR from FG_PROCESS_MACH_MAP where REF_PROCESS_ID in (processid)))*1.0 as 'Efficiency'  from dailyreport  where rdate >= '"+formatFromDate ()+"' AND rdate <= '"+formatToDate ()+"' AND  empid = "+employee );
                jTable1.setModel ( buildTableModel ( result , reportType , "xAxisLabel" ) );

                generateBarChart ( "Year to Date Productivity " , "YEAR" , "PRODUCTION" );

            } else if ( reportType.equalsIgnoreCase ( "Daily" ) ) {

                //result = DB_Operations.executeSingle ("select (select PROCESS_NAME from PROCESS_MASTER where PROCESS_ID in (processid)) as 'Process' , (select PART_NAME from finished_goods where FG_ID in (fgid)) as 'Product',  showrdate  as 'Start Date', SUM(qtyout) as 'Output Qty',    rdate,                  (qtyout/(stoptime-starttime)) as 'Speed', (stoptime-starttime) as 'Hrs', (qtyin-qtyout) as 'Balence' from dailyreport where fgid="+product+" group by rdate");
                result = DB_Operations.executeSingle ( "select (select PROCESS_NAME from PROCESS_MASTER where PROCESS_ID in (processid)) as 'Process', (select TG_OPT_HR from FG_PROCESS_MACH_MAP where REF_PROCESS_ID in (processid)) as 'Expected',  rdate, SUM( showToTime - showFromTime) as 'Work Hours', qtyout, ( qtyout / SUM(showToTime - showFromTime))*1.0 as 'Hourly Output' , (( qtyout / SUM(showToTime - showFromTime)) / (select TG_OPT_HR from FG_PROCESS_MACH_MAP where REF_PROCESS_ID in (processid)))*1.0 as 'Efficiency'  from dailyreport  where rdate >= '"+formatFromDate ()+"' AND rdate <= '"+formatToDate ()+"' AND  empid = "+employee+" group by rdate" );
                jTable1.setModel ( buildTableModel ( result , reportType , "xAxisLabel" ) );

                generateBarChart ( "Daily productivity report for period " + sdfTS.format ( c1.getTime () ) + " to " + sdfTS.format ( c2.getTime () ) , "DAYS" , "PRODUCTION" );
                ...
                for ( int i = 0, j = i + 1 ; j < weeklyDates.size () ; i = i + 2 , j += 2 ) {
                    ...
                    //result = DB_Operations.executeSingle ("select (select PROCESS_NAME from PROCESS_MASTER where PROCESS_ID in (processid)) as 'Process' , (select PART_NAME from finished_goods where FG_ID in (fgid)) as 'Product',  showrdate  as 'Start Date', SUM(qtyout) as 'Output Qty',    rdate,                  (qtyout/(stoptime-starttime)) as 'Speed', (stoptime-starttime) as 'Hrs', (qtyin-qtyout) as 'Balence' from dailyreport where fgid="+product+" group by rdate");
                    result = DB_Operations.executeSingle ( "select (select PROCESS_NAME from PROCESS_MASTER where PROCESS_ID in (processid)) as 'Process', (select TG_OPT_HR from FG_PROCESS_MACH_MAP where REF_PROCESS_ID in (processid)) as 'Expected',  rdate, SUM( showToTime - showFromTime) as 'Work Hours', qtyout, ( qtyout / SUM(showToTime - showFromTime))*1.0 as 'Hourly Output' , (( qtyout / SUM(showToTime - showFromTime)) / (select TG_OPT_HR from FG_PROCESS_MACH_MAP where REF_PROCESS_ID in (processid)))*1.0 as 'Efficiency'  from dailyreport  where rdate >= '"+weeklyDates.get(i)+"' AND rdate <= "+weeklyDates.get(j)+"' AND  empid = "+employee );
                    fromWeekDate = weeklyDates.get ( i ).split ( " " )[ 0 ].substring ( 5 );
                    toWeekDate = weeklyDates.get ( j ).split ( " " )[ 0 ].substring ( 5 );
                    jTable1.setModel ( buildTableModel ( result , reportType , fromWeekDate + " to " + toWeekDate ) );
                }

                generateBarChart ( "Weekly productivity report for period " + weeklyDates.get ( 0 ).split ( " " )[ 0 ] + " to " + sdfTS.format ( c2.getTime () ) , "WEEKS" , "PRODUCTION" );
                ...
                for ( int i = 0, j = i + 1, k = 0 ; j < months.size () && k < 12 ; i = i + 2 , j += 2 , k ++ ) {
                    ...
                    //result = DB_Operations.executeSingle ("select (select PROCESS_NAME from PROCESS_MASTER where PROCESS_ID in (processid)) as 'Process' , (select PART_NAME from finished_goods where FG_ID in (fgid)) as 'Product',  showrdate  as 'Start Date', SUM(qtyout) as 'Output Qty',    rdate,                  (qtyout/(stoptime-starttime)) as 'Speed', (stoptime-starttime) as 'Hrs', (qtyin-qtyout) as 'Balence' from dailyreport where fgid="+product+" group by rdate");
                    result = DB_Operations.executeSingle ( "select (select PROCESS_NAME from PROCESS_MASTER where PROCESS_ID in (processid)) as 'Process', (select TG_OPT_HR from FG_PROCESS_MACH_MAP where REF_PROCESS_ID in (processid)) as 'Expected',  rdate, SUM( showToTime - showFromTime) as 'Work Hours', qtyout, ( qtyout / SUM(showToTime - showFromTime))*1.0 as 'Hourly Output' , (( qtyout / SUM(showToTime - showFromTime)) / (select TG_OPT_HR from FG_PROCESS_MACH_MAP where REF_PROCESS_ID in (processid)))*1.0 as 'Efficiency'  from dailyreport  where rdate >= '" + months.get ( i ) + "' AND rdate <= '" + months.get ( j ) + "'  AND empid=" + employee );
                    jTable1.setModel ( buildTableModel ( result , reportType , monthsTitles[ k ] ) );
                }

                generateBarChart ( "Monthly Productivity report for year " + year + " - " + ( Integer.parseInt ( year ) + 1 ) , "MONTHS" , "PRODUCTION" );
                ...
                for ( int i = 0, j = i + 1, k = 0 ; j < quarters.size () && k < 4 ; i = i + 2 , j += 2 , k ++ ) {
                    ...
                    //result = DB_Operations.executeSingle ("select (select PROCESS_NAME from PROCESS_MASTER where PROCESS_ID in (processid)) as 'Process' , (select PART_NAME from finished_goods where FG_ID in (fgid)) as 'Product',  showrdate  as 'Start Date', SUM(qtyout) as 'Output Qty',    rdate,                  (qtyout/(stoptime-starttime)) as 'Speed', (stoptime-starttime) as 'Hrs', (qtyin-qtyout) as 'Balence' from dailyreport where fgid="+product+" group by rdate");
                    result = DB_Operations.executeSingle ( "select (select PROCESS_NAME from PROCESS_MASTER where PROCESS_ID in (processid)) as 'Process', (select TG_OPT_HR from FG_PROCESS_MACH_MAP where REF_PROCESS_ID in (processid)) as 'Expected',  rdate, SUM( showToTime - showFromTime) as 'Work Hours', qtyout, ( qtyout / SUM(showToTime - showFromTime))*1.0 as 'Hourly Output' , (( qtyout / SUM(showToTime - showFromTime)) / (select TG_OPT_HR from FG_PROCESS_MACH_MAP where REF_PROCESS_ID in (processid)))*1.0 as 'Efficiency'  from dailyreport  where rdate >= '" + quarters.get ( i ) + "' AND rdate <= '" + quarters.get ( j ) + "'  AND empid=" + employee );
                    jTable1.setModel ( buildTableModel ( result , reportType , quarterTitles[ k ] ) );
                }

                generateBarChart ( "Quarterly productivity report for year " + year + " - " + ( Integer.parseInt ( year ) + 1 ) , "Quarters" , "PRODUCTION" );
...
//                    }
    ...
    private void initComponents() {
        ...
        try {
            dateChooserCombo3.setDefaultPeriods(new datechooser.model.multiple.PeriodSet(new datechooser.model.multiple.Period(new java.util.GregorianCalendar(2018, 9, 19),
                new java.util.GregorianCalendar(2018, 9, 19))));
    } catch (datechooser.model.exeptions.IncompatibleDataExeption e1) {
        e1.printStackTrace();
    }
    add(dateChooserCombo3);
    dateChooserCombo3.setBounds(30, 130, 155, 30);

    dateChooserCombo5.setCalendarBackground(new java.awt.Color(204, 255, 51));
    dateChooserCombo5.setCalendarPreferredSize(new java.awt.Dimension(330, 230));
    add(dateChooserCombo5);
    dateChooserCombo5.setBounds(30, 70, 155, 30);
    ...
    public String formatFromDate () {
        ...
        String[] dateIP = dateChooserCombo5.getText ().split ( " " );
...
//          
    ...
    private void jButton1MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jButton1MouseClicked
        ...
        //jCheckBox4
        //process
        if (   jCheckBox1.isSelected () &&  ! jCheckBox4.isSelected () &&   jCheckBox2.isSelected () &&   jCheckBox3.isSelected () &&   jCheckBox5.isSelected ()  ) {

            //for a product     for a process       for a  machine      for an employee
            //loadEntriesAllSelected ( reportType , product , process , machine , employee, shift);
            loadEntriesAllMachine ( reportType , employee );
...
//
//            //for a product     for a process       for a  machine      for an employee
//            loadEntriesAllMachine ( reportType , product , employee );
//
//        } else if (  ! jCheckBox1.isSelected () && jCheckBox4.isSelected () && jCheckBox2.isSelected () && jCheckBox3.isSelected () &&  ! jCheckBox5.isSelected () ) {
//
//            //for a product     for a process       for a  machine      for an employee
...
//            loadEntriesNoFilters ( reportType );
//        }
