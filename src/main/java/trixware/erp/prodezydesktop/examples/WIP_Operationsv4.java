 ...
 * @author Rajesh
 */
public class WIP_Operationsv4 extends javax.swing.JPanel {
    ...
    String selectectedOrder, selectectedCustomer, selectedProduct;

    /**
     * Creates new form WIP_Operations
     */
    public WIP_Operationsv4 () {
        initComponents ();
    ...
    public void loadProductsAndQtys ( String selctOrder ) {
        ...
        try {
            ...
            result = DB_Operations.executeSingle ( "SELECT (SELECT distinct(CUSTOMER_NAME) from customer where CUSTOMER_ID in (so_customer_id)) as CustomerName  FROM sales_orders where sales_po_id = " + selectectedOrder );
            if ( result != null ) {
    ...
    private void initComponents() {
        ...
        add(jComboBox1);
        jComboBox1.setBounds(370, 50, 180, 40);

        add(jComboBox2);
        jComboBox2.setBounds(80, 170, 180, 40);

        jTextField1.setEditable(false);
        jTextField1.setText("0");
        jTextField1.setToolTipText("Total Qty");
        add(jTextField1);
        jTextField1.setBounds(370, 110, 180, 40);

        jLabel1.setFont(new java.awt.Font("Leelawadee UI", 0, 12)); // NOI18N
        jLabel1.setText("Move to process");
        add(jLabel1);
        jLabel1.setBounds(680, 30, 180, 16);
        ...
        add(jComboBox3);
        jComboBox3.setBounds(680, 50, 180, 40);

        jLabel2.setFont(new java.awt.Font("Leelawadee UI", 0, 12)); // NOI18N
        jLabel2.setText("Nos");
        add(jLabel2);
        jLabel2.setBounds(820, 120, 40, 20);

        jTextField2.setText("0");
        jTextField2.setToolTipText("Move");
        add(jTextField2);
        jTextField2.setBounds(680, 110, 180, 40);
        ...
        add(jLabel3);
        jLabel3.setBounds(370, 90, 70, 20);
        ...
        add(jCheckBox1);
        jCheckBox1.setBounds(680, 170, 180, 24);
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        if ( MoveToStores ) {
            ...
            selectedProduct = products.get ( jComboBox1.getSelectedIndex () )[ 0 ];
            int MOVEQTY = 0;
            int REJECTQTY = 0;
            ...
            updateStock ( selectedProduct , selectectedOrder, MOVEQTY);
    ...
    public void updateWIP () {
        ...
        if ( MOVEQTY > 0 ) {
            ...
            try {
                ...
                System.out.println ( "Number Format Wrong 1" + nef2.getMessage () );
            } catch ( Exception nef2 ) {
                System.out.println ( "Error 2" + nef2.getMessage () );
            }
        ...
        if ( REJECTQTY > 0 ) {
            ...
            try {
                ...
                try{pastProcesses.close();}catch(Exception w ){ }
    ...
    public void updateStock (String a, String b, int qty) {
        
        String query = "select MAX(FPM_ID), REF_PROCESS_ID from FG_PROCESS_MACH_MAP where REF_FG_ID = '"+ a+"'" ;
        
        c2 = Calendar.getInstance ();
        sdf2 = new SimpleDateFormat ( "dd-MM-yyyy HH:mm:ss" );
        String strDate2 = sdf2.format ( c2.getTime () );

        ResultSet rs = DB_Operations.executeSingle (  query  );
        try{
     
            int lastProcessID = Integer.parseInt( rs.getString("REF_PROCESS_ID") );
            
            String test2 = "update op_wip set  MOVEDATE= '" + strDate2 + "'  ,  MOVEQTY =  (MOVEQTY - " + qty + ")  where PROCESS_STAGE = " + lastProcessID + " AND MOVEQTY > 0";
            DB_Operations.executeInsertRandom ( test2 );
            
            test2 = "update po_order_details set   finished_qty =  (finished_qty + " + qty + ")  where ref_po_id = " + b +" AND product_id= "+1 ;
            DB_Operations.executeInsertRandom ( test2 );
        
        }catch( SQLException e){
            System.out.println ( "Update Stock Exception 1" + e.getMessage () );
        }catch( Exception e){
            System.out.println ( "Update Stock Exception 2" + e.getMessage () );
        }
...
//        }
    ...
    public void loadOrders ( String selectedOrder , String selectedProduct ) {
        ...
        try {
            ...
            loadOrderDetailsIfAvail = "SELECT MOVEDATE,  MOVEQTY, PROCESS_STAGE, product_qty as TARGETQTY,  REJECTED AS Rejected   FROM op_wip a INNER JOIN po_order_details b WHERE   PRODUCT =" + selectedProduct + " AND ORDERNO = " + selectedOrder + " AND a.PRODUCT = b.product_id  AND a.ORDERNO = b.ref_po_id";

            orderDetails = DB_Operations.executeSingle ( loadOrderDetailsIfAvail );

            jTable1.setModel ( buildTableModel2 ( orderDetails, selectedOrder, selectedProduct ) );
            ...
            try{result2.close();}catch(Exception w ){ }
            try{orderDetails.close();}catch(Exception w ){ }
    ...
    public DefaultTableModel buildTableModel2 ( ResultSet rs, String a, String  b )
        ...
        columnNames.add ( "<html>Not processed<html>" );
        ...
        ResultSet rs1 = null ;
        try{
            String query = "select finished_qty from po_order_details where ref_po_id = '"+  a +"' AND product_id = '"+  b +" ' " ;
             rs1 = DB_Operations.executeSingle ( query) ;

             vector.add ( "<html> <b>" + rs1.getString ( "finished_qty" ) + "</html>" );
