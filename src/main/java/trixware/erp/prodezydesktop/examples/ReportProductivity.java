 ...
 * @author Rajesh
 */
public class ReportProductivity extends javax.swing.JPanel {
...
//                                }
     ...
     public void loadEntriesAllSelected ( String reportType , int product, int process, int machine, int employee ) {
        ...
        try {
            ...
            if ( reportType.equalsIgnoreCase ( "Yearly" ) ) {
                ...
                //result = DB_Operations.executeSingle ("select (select PROCESS_NAME from PROCESS_MASTER where PROCESS_ID in (processid)) as 'Process' , (select PART_NAME from finished_goods where FG_ID in (fgid)) as 'Product',    showrdate  as 'Start Date',     SUM(qtyout) as 'Output Qty',     rdate,        SUM(qtyout/(stoptime-starttime)) as 'Speed', SUM(stoptime-starttime) as 'Hrs', SUM(qtyin-qtyout) as 'Balence' from dailyreport where rdate >= '"+"01-04-2018"+"' AND rtdate <= '"+"31-03-2018"+"' AND fgid="+product);
                result = DB_Operations.executeSingle ( "select (select PART_NAME from finished_goods where FG_ID in (fgid)) as 'Product',    showrdate  as 'Start Date',     SUM(qtyout) as 'Output Qty',   SUM(qtyout/(stoptime-starttime)) as 'Speed', SUM(stoptime-starttime) as 'Hrs', SUM(qtyin-qtyout) as 'Balence' from dailyreport where rdate >= '" + formatFromDate () + "' AND rdate <= '" + formatToDate () + "' AND fgid=" + product+ " AND processid=" + process+ " AND machineid=" + machine+ " AND empid=" + employee );
                jTable1.setModel ( buildTableModel ( result , reportType , "xAxisLabel" ) );
                ...
                //result = DB_Operations.executeSingle ("select (select PROCESS_NAME from PROCESS_MASTER where PROCESS_ID in (processid)) as 'Process' , (select PART_NAME from finished_goods where FG_ID in (fgid)) as 'Product',  showrdate  as 'Start Date', SUM(qtyout) as 'Output Qty',    rdate,                  (qtyout/(stoptime-starttime)) as 'Speed', (stoptime-starttime) as 'Hrs', (qtyin-qtyout) as 'Balence' from dailyreport where fgid="+product+" group by rdate");
                result = DB_Operations.executeSingle ( "select (select PART_NAME from finished_goods where FG_ID in (fgid)) as 'Product',  showrdate  as 'Start Date', SUM(qtyout) as 'Output Qty',   (qtyout/(stoptime-starttime)) as 'Speed', (stoptime-starttime) as 'Hrs', (qtyin-qtyout) as 'Balence' from dailyreport where  rdate >= '" + formatFromDate () + "' AND rdate <= '" + formatToDate () + "' AND fgid=" + product + " AND processid=" + process+ " AND machineid=" + machine+ " AND empid=" + employee+ " group by rdate" );
                jTable1.setModel ( buildTableModel ( result , reportType , "xAxisLabel" ) );
                ...
                for ( int i = 0, j = i + 1 ; j < weeklyDates.size () ; i = i + 2 , j += 2 ) {
                    ...
                    //result = DB_Operations.executeSingle ("select (select PROCESS_NAME from PROCESS_MASTER where PROCESS_ID in (processid)) as 'Process' , (select PART_NAME from finished_goods where FG_ID in (fgid)) as 'Product',  showrdate  as 'Start Date', SUM(qtyout) as 'Output Qty',    rdate,                  (qtyout/(stoptime-starttime)) as 'Speed', (stoptime-starttime) as 'Hrs', (qtyin-qtyout) as 'Balence' from dailyreport where fgid="+product+" group by rdate");
                    result = DB_Operations.executeSingle ( "select (select PART_NAME from finished_goods where FG_ID in (fgid)) as 'Product',  showrdate  as 'Start Date', SUM(qtyout) as 'Output Qty',   (qtyout/(stoptime-starttime)) as 'Speed', (stoptime-starttime) as 'Hrs', (qtyin-qtyout) as 'Balence' from dailyreport where  rdate >= '" + weeklyDates.get ( i ) + "' AND rdate <= '" + weeklyDates.get ( j ) + "' AND fgid=" + product + " AND processid=" + process+ " AND machineid=" + machine+ " AND empid=" + employee);
                    fromWeekDate = weeklyDates.get ( i ).split ( " " )[ 0 ].substring ( 5 );
                    toWeekDate = weeklyDates.get ( j ).split ( " " )[ 0 ].substring ( 5 );
                    jTable1.setModel ( buildTableModel ( result , reportType , fromWeekDate + " to " + toWeekDate ) );
                }
                ...
                for ( int i = 0, j = i + 1, k = 0 ; j < months.size () && k < 12 ; i = i + 2 , j += 2 , k ++ ) {
                    ...
                    //result = DB_Operations.executeSingle ("select (select PROCESS_NAME from PROCESS_MASTER where PROCESS_ID in (processid)) as 'Process' , (select PART_NAME from finished_goods where FG_ID in (fgid)) as 'Product',  showrdate  as 'Start Date', SUM(qtyout) as 'Output Qty',    rdate,                  (qtyout/(stoptime-starttime)) as 'Speed', (stoptime-starttime) as 'Hrs', (qtyin-qtyout) as 'Balence' from dailyreport where fgid="+product+" group by rdate");
                    result = DB_Operations.executeSingle ( "select (select PART_NAME from finished_goods where FG_ID in (fgid)) as 'Product',  showrdate  as 'Start Date', SUM(qtyout) as 'Output Qty',   (qtyout/(stoptime-starttime)) as 'Speed', (stoptime-starttime) as 'Hrs', (qtyin-qtyout) as 'Balence' from dailyreport where  rdate >= '" + months.get ( i ) + "' AND rdate <= '" + months.get ( j ) + "' AND fgid=" + product + " AND processid=" + process+ " AND machineid=" + machine+ " AND empid=" + employee);
                    jTable1.setModel ( buildTableModel ( result , reportType , monthsTitles[ k ] ) );
                }
                ...
                for ( int i = 0, j = i + 1, k = 0 ; j < quarters.size () && k < 4 ; i = i + 2 , j += 2 , k ++ ) {
                    ...
                    //result = DB_Operations.executeSingle ("select (select PROCESS_NAME from PROCESS_MASTER where PROCESS_ID in (processid)) as 'Process' , (select PART_NAME from finished_goods where FG_ID in (fgid)) as 'Product',  showrdate  as 'Start Date', SUM(qtyout) as 'Output Qty',    rdate,                  (qtyout/(stoptime-starttime)) as 'Speed', (stoptime-starttime) as 'Hrs', (qtyin-qtyout) as 'Balence' from dailyreport where fgid="+product+" group by rdate");
                    result = DB_Operations.executeSingle ( "select (select PART_NAME from finished_goods where FG_ID in (fgid)) as 'Product',  showrdate  as 'Start Date', SUM(qtyout) as 'Output Qty',   (qtyout/(stoptime-starttime)) as 'Speed', (stoptime-starttime) as 'Hrs', (qtyin-qtyout) as 'Balence' from dailyreport where  rdate >= '" + quarters.get ( i ) + "' AND rdate <= '" + quarters.get ( j ) + "' AND fgid=" + product + " AND processid=" + process+ " AND machineid=" + machine+ " AND empid=" + employee);
                    jTable1.setModel ( buildTableModel ( result , reportType , quarterTitles[ k ] ) );
                }
        ...
        try {
            ...
            if ( reportType.equalsIgnoreCase ( "Yearly" ) ) {
              ...
              // select (select PROCESS_NAME from PROCESS_MASTER where PROCESS_ID in (processid)) as 'Process', (select TG_OPT_HR from FG_PROCESS_MACH_MAP where REF_PROCESS_ID in (processid)) as 'Expected',  rdate, SUM( showToTime - showFromTime) as 'Work Hours', qtyout, ( qtyout / SUM(showToTime - showFromTime))*1.0 as 'Hourly Output' , (( qtyout / SUM(showToTime - showFromTime)) / (select TG_OPT_HR from FG_PROCESS_MACH_MAP where REF_PROCESS_ID in (processid)))*1.0 as 'Efficiency'  from dailyreport where empid = 1 group by rdate  
                result = DB_Operations.executeSingle ( "select showrdate as 'Date',  (select TG_OPT_HR from FG_PROCESS_MACH_MAP where REF_PROCESS_ID in (processid)) as 'Expected O/P per hr',   SUM( showToTime - showFromTime) as 'Work Hours', qtyout as ' Qty Produced', ( qtyout / SUM(showToTime - showFromTime)) as 'Actual O/p per hr' , (( qtyout / SUM(showToTime - showFromTime)) / (select TG_OPT_HR from FG_PROCESS_MACH_MAP where REF_PROCESS_ID in (processid)))*100 as 'Efficiency'  from dailyreport  where rdate >= '"+formatFromDate ()+"' AND rdate <= '"+formatToDate ()+"' AND  empid = "+employee );
                jTable1.setModel ( buildTableModel ( result , reportType , "xAxisLabel" ) );

                generateBarChart ( "Year to Date Productivity " , "YEAR" , "Productivity" );
                ...
                generateBarChart ( "Daily productivity report for period " + sdfTS.format ( c1.getTime () ) + " to " + sdfTS.format ( c2.getTime () ) , "DAYS" , "Productivity" );
                ...
                for ( int i = 0, j = i + 1 ; j < weeklyDates.size () ; i = i + 2 , j += 2 ) {
                    ...
                    //result = DB_Operations.executeSingle ("select (select PROCESS_NAME from PROCESS_MASTER where PROCESS_ID in (processid)) as 'Process' , (select PART_NAME from finished_goods where FG_ID in (fgid)) as 'Product',  showrdate  as 'Start Date', SUM(qtyout) as 'Output Qty',    rdate,                  (qtyout/(stoptime-starttime)) as 'Speed', (stoptime-starttime) as 'Hrs', (qtyin-qtyout) as 'Balence' from dailyreport where fgid="+product+" group by rdate");
                    result = DB_Operations.executeSingle ( "select  showrdate as 'Date', (select TG_OPT_HR from FG_PROCESS_MACH_MAP where REF_PROCESS_ID in (processid)) as 'Expected O/P per hr',   SUM( showToTime - showFromTime) as 'Work Hours', qtyout as ' Qty Produced', ( qtyout / SUM(showToTime - showFromTime)) as 'Actual O/p per hr' , (( qtyout / SUM(showToTime - showFromTime)) / (select TG_OPT_HR from FG_PROCESS_MACH_MAP where REF_PROCESS_ID in (processid)))*100 as 'Efficiency'  from dailyreport  where rdate >= '"+weeklyDates.get(i)+"' AND rdate <= '"+weeklyDates.get(j)+"' AND  empid = "+employee );
                    fromWeekDate = weeklyDates.get ( i ).split ( " " )[ 0 ].substring ( 5 );
                    toWeekDate = weeklyDates.get ( j ).split ( " " )[ 0 ].substring ( 5 );
                    jTable1.setModel ( buildTableModel ( result , reportType , fromWeekDate + " to " + toWeekDate ) );
                }

                generateBarChart ( "Weekly productivity report for period " + weeklyDates.get ( 0 ).split ( " " )[ 0 ] + " to " + sdfTS.format ( c2.getTime () ) , "WEEKS" , "Productivity" );
                ...
                for ( int i = 0, j = i + 1, k = 0 ; j < months.size () && k < 12 ; i = i + 2 , j += 2 , k ++ ) {
                    ...
                    //result = DB_Operations.executeSingle ("select (select PROCESS_NAME from PROCESS_MASTER where PROCESS_ID in (processid)) as 'Process' , (select PART_NAME from finished_goods where FG_ID in (fgid)) as 'Product',  showrdate  as 'Start Date', SUM(qtyout) as 'Output Qty',    rdate,                  (qtyout/(stoptime-starttime)) as 'Speed', (stoptime-starttime) as 'Hrs', (qtyin-qtyout) as 'Balence' from dailyreport where fgid="+product+" group by rdate");
                    result = DB_Operations.executeSingle ( "select  showrdate as 'Date', (select TG_OPT_HR from FG_PROCESS_MACH_MAP where REF_PROCESS_ID in (processid)) as 'Expected O/P per hr',   SUM( showToTime - showFromTime) as 'Work Hours', qtyout as ' Qty Produced', ( qtyout / SUM(showToTime - showFromTime)) as 'Actual O/p per hr' , (( qtyout / SUM(showToTime - showFromTime)) / (select TG_OPT_HR from FG_PROCESS_MACH_MAP where REF_PROCESS_ID in (processid)))*100 as 'Efficiency'  from dailyreport  where rdate >= '" + months.get ( i ) + "' AND rdate <= '" + months.get ( j ) + "'  AND empid=" + employee );
                    jTable1.setModel ( buildTableModel ( result , reportType , monthsTitles[ k ] ) );
                }

                generateBarChart ( "Monthly Productivity report for year " + year + " - " + ( Integer.parseInt ( year ) + 1 ) , "MONTHS" , "Productivity" );
                ...
                for ( int i = 0, j = i + 1, k = 0 ; j < quarters.size () && k < 4 ; i = i + 2 , j += 2 , k ++ ) {
                    ...
                    //result = DB_Operations.executeSingle ("select (select PROCESS_NAME from PROCESS_MASTER where PROCESS_ID in (processid)) as 'Process' , (select PART_NAME from finished_goods where FG_ID in (fgid)) as 'Product',  showrdate  as 'Start Date', SUM(qtyout) as 'Output Qty',    rdate,                  (qtyout/(stoptime-starttime)) as 'Speed', (stoptime-starttime) as 'Hrs', (qtyin-qtyout) as 'Balence' from dailyreport where fgid="+product+" group by rdate");
                    result = DB_Operations.executeSingle ( "select  showrdate as 'Date', (select TG_OPT_HR from FG_PROCESS_MACH_MAP where REF_PROCESS_ID in (processid)) as 'Expected O/P per hr',  SUM( showToTime - showFromTime) as 'Work Hours', qtyout as ' Qty Produced', ( qtyout / SUM(showToTime - showFromTime)) as 'Actual O/p per hr' , (( qtyout / SUM(showToTime - showFromTime)) / (select TG_OPT_HR from FG_PROCESS_MACH_MAP where REF_PROCESS_ID in (processid)))*100 as 'Efficiency'  from dailyreport  where rdate >= '" + quarters.get ( i ) + "' AND rdate <= '" + quarters.get ( j ) + "'  AND empid=" + employee );
                    jTable1.setModel ( buildTableModel ( result , reportType , quarterTitles[ k ] ) );
                }

                generateBarChart ( "Quarterly productivity report for year " + year + " - " + ( Integer.parseInt ( year ) + 1 ) , "Quarters" , "Productivity" );
    ...
    private void initComponents() {
        ...
        add(jComboBox1);
        jComboBox1.setBounds(40, 250, 160, 30);
        ...
        add(jComboBox2);
        jComboBox2.setBounds(40, 370, 160, 30);
        ...
        add(jComboBox3);
        jComboBox3.setBounds(40, 430, 160, 30);
        ...
        add(jComboBox5);
        jComboBox5.setBounds(40, 170, 160, 30);
        ...
        add(jCheckBox1);
        jCheckBox1.setBounds(210, 250, 80, 24);
        ...
        add(jCheckBox2);
        jCheckBox2.setBounds(210, 310, 76, 24);
        ...
        add(jCheckBox3);
        jCheckBox3.setBounds(210, 370, 76, 24);
        ...
        add(jCheckBox4);
        jCheckBox4.setBounds(210, 430, 76, 24);
